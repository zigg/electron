...
void DevToolsFileSystemIndexer::FileSystemIndexingJob::Start() {
  ...
  impl_task_runner()->PostTask(
      FROM_HERE, Bind(&FileSystemIndexingJob::CollectFilesToIndex, this));
}

void DevToolsFileSystemIndexer::FileSystemIndexingJob::Stop() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  impl_task_runner()->PostTask(
      FROM_HERE, Bind(&FileSystemIndexingJob::StopOnImplSequence, this));
}
...
void DevToolsFileSystemIndexer::SearchInPath(const string& file_system_path,
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  impl_task_runner()->PostTask(
      FROM_HERE, Bind(&DevToolsFileSystemIndexer::SearchInPathOnImplSequence,
                      this, file_system_path, query, callback));
}
